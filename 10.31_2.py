...
class TemporalFlowManager:
    ...
    def _calculate_start_times(self):
        ...
        for i in range(1, len(self.temporal_flows)):
            ...
            if T_prev > T_current:
                # 情况A：前驱流持续时间更长
                current_start = prev_start + (T_prev - T_current) + prev_flow.period_us
                #current_start = prev_start + (T_prev - T_current) +  self.safety_margin_us
                reason = f"T_prev({T_prev:.1f}us) > T_current({T_current:.1f}us)"
            else:
                # 情况B：当前流持续时间更长或相等
                current_start = prev_start +   prev_flow.period_us
                #current_start = prev_start +  self.safety_margin_us
                reason = f"T_prev({T_prev:.1f}us) <= T_current({T_current:.1f}us)"
