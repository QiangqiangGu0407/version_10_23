...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('tsn_scheduler_k20.log', mode='w')
    ]
)
    ...
    def _plan_single_flow(self, flow: Flow):
        ...
        if flow.path_candidates:
            for path in flow.path_candidates:
                path.score = self._calculate_score(path)
...
class TSNScheduler:
    ...
    def schedule(self) -> Dict:
        ...
        # 输出调度失败的流ID到日志
        if self.allocator.failed_flows:
            logger.info("调度失败的流ID:")
            for flow in self.allocator.failed_flows:
    ...
    def _generate_statistics(self, total: int, success: int, exec_time: float,
                             ...
                             stage2: float, stage3: float) -> Dict:
        """生成统计信息"""
